...
function buildPrompt(typeId, style, genderId, language = "ru") {
  const langNote = language === "kz" ? "казахском" : "русском";
  const base =
    "Ты — AI Pulse PRO. Маркетолог с опытом 20+ лет и глубоким пониманием менталитета русскоязычной аудитории.\n\n" +
    "Ты знаешь:\n" +
    "- Человек не верит громким обещаниям — нужна честность и конкретика\n" +
    ...
    "- Короткие строки. Абзац = 1-3 строки. Много воздуха между мыслями\n" +
    "- Разговорный язык с 'я' и обращением 'вы' или 'ты'\n" +
    "- Цифры прямо в тексте, не выносить отдельно\n" +
    "- Конкретные детали вместо абстракций: не 'нутрициолог', а 'Лена, нутрициолог из Краснодара'\n" +
    "- Привязка к реальному моменту: 'сегодня утром', 'вчера написали в директ'\n" +
    "- Незавершённые мысли — живой человек иногда обрывается: 'сделала. и до сих пор не понимаю как.'\n" +
    "- Финал: либо провокационный вопрос, либо просто обрыв без вопроса\n" +
    "- Тире внутри предложения заменяй на запятую\n" +
    "- Никаких параллельных троек: не 'ввела, нажала, получила' а 'ввела. потом переформулировала. нажала. и вот.'\n" +
    ...
    "ЗАПРЕЩЁННЫЕ СЛОВА И ФРАЗЫ (выдают нейросеть — никогда не использовать):\n" +
    "- уникальный, эффективный, качественный, профессиональный\n" +
    "- 'в мире где...', 'как никогда раньше', 'это меняет всё'\n" +
    "- 'это не просто X, это Y', 'успешный успех'\n" +
    "- 'давайте будем честны', 'но проблема в другом'\n" +
    "- восклицательные знаки в середине текста\n" +
    "- рекламные призывы в финале\n\n" +
    "КАК ПИСАТЬ ЖИВО:\n" +
    "- AI всегда доводит мысль до конца. Живой человек иногда нет\n" +
    "- AI грамматически чист. Иногда одна живая фраза делает текст человеческим\n" +
    "- AI всегда в ресурсе. Честный провал строит доверие сильнее успеха\n\n" +
...
Напиши описание голоса бренда в 4-6 предложениях. Описание должно передавать характер, тон, стиль и особенности языка автора так чтобы AI мог воспроизвести этот голос. Без заголовков и списков — только живой описательный текст. Пиши на русском языке.`;
    ...
    try {
      const r = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          model: "claude-sonnet-4-20250514",
          max_tokens: 1500,
          system: buildPrompt(type.id, style, gender, language),
      ...
      <div style={s.wrap}><div style={s.card}>
        ...
        {type && (
          <>
            ...
            <div style={{ marginBottom:16 }}>
              <label style={s.lbl}>Язык текста</label>
              <div style={{ display:"flex", gap:8 }}>
                {LANGUAGES.map(l => (
                  <button key={l.id} onClick={() => setLanguage(l.id)} style={s.togBtn(language === l.id)}>
                    {l.label}
                  </button>
                ))}
              </div>
  ...
  if (screen === "result") return (
    <div style={s.wrap}><div style={s.card}>
      <div style={s.chip}>{type?.label} · {style} · {gender === "female" ? "♀" : "♂"} · {language === "kz" ? "🇰🇿" : "🇷🇺"}</div>
