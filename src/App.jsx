...
function buildSystemPrompt(typeId, style) {
  const base =
    "Ты — AI Pulse PRO. Ты маркетолог с опытом 20+ лет, глубоким пониманием русского менталитета, психологии и мотивации русскоязычной аудитории.\n\n" +
    "Ты знаешь:\n" +
  ...
  const prompts = {
    ...
    reels:
      ...
      "ФОРМАТ: пиши как сценарий — что говорить, что показывать, текст на экране\n" +
      "ВАЖНО: первые 3 секунды останавливают палец — хук решает всё\n",

    promo:
      "\nТЫ ПИШЕШЬ: серию для прогрева на 6 дней\n" +
      "СТРУКТУРА:\n" +
      ...
      "День 5: Решение — продукт через результат клиента или кейс\n" +
      "День 6: Оффер — мягкое закрытие с ограничением\n" +
      "ВАЖНО: каждый день заканчивается вопросом или интригой\n",
    ...
    plan:
      ...
      "ВАЖНО: темы должны цеплять и вести к продаже\n",

    niche:
      "\nТЫ ПРОВОДИШЬ: полный анализ ниши\n" +
      "СТРУКТУРА:\n" +
      "Портрет аудитории — кто эти люди, что тревожит, о чём мечтают\n" +
    ...
    strategy:
      ...
      "Точка Б — что хочет получить через 3-6 месяцев\n" +
      ...
      "Метрики — победа через 30/60/90 дней\n" +
      "ВАЖНО: как наставник который прошёл этот путь. Цифры, сроки, конкретика\n",
  };
...
export default function App() {
  ...
  const topicLabel = ["niche", "plan"].includes(type?.id) ? "Ниша / тема бизнеса *" : type?.id === "strategy" ? "Ниша / тема блога *" : "Тема / о чём текст *";
  const topicPlaceholder = ["niche", "plan"].includes(type?.id) ? "Например: нутрициология, психология, коучинг…" : type?.id === "strategy" ? "Например: блог психолога, эксперт по деньгам…" : "Например: курс по инвестициям для мам в декрете…";
      ...
      {loading ? <Spinner /> : result === "__error__" ? (
        <div style={{ textAlign: "center", padding: "24px 0" }}>
          ...
          <div style={{ fontSize: 13, color: "#6b7280", marginBottom: 24, lineHeight: 1.6 }}>
            Возможно, слабый интернет.<br />Попробуй ещё раз.
          </div>
